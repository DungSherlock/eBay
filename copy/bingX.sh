...
linkPost=$linkApi`echo nickName==IMPORTDATA\(\"`$linkGit`echo nickName.txt\"\)\&uid==IMPORTDATA\(\"`$linkGit`echo uid.txt\"\)\&equity==IMPORTDATA\(\"`$linkGit`echo equity.txt\"\)\&totalEarnings==IMPORTDATA\(\"`$linkGit`echo totalEarnings.txt\"\)`
echo $linkPost
