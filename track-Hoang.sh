...
: > track-Hoang-fullName-CO.txt
: > track-Hoang-address-CO.txt
: > track-Hoang-status-CO.txt
: > track-Hoang-number-CO.txt
: > track-Hoang-url-CO.txt
: > track-Hoang-OrderID-CO.txt
...
do
  ...
  while true
    ...
    if [[ $curl =~ "error" ]]; then
        echo "1-Sai info"
        echo ${array[2]} >> track-Hoang-OrderID-CO.txt
        echo ${array[1]} >> track-Hoang-email-CO.txt
        echo "Sai info" >> track-Hoang-usItemId-CO.txt
        echo "Sai info" >> track-Hoang-priceValue-CO.txt
        echo "Sai info" >> track-Hoang-fullName-CO.txt
        echo "Sai info" >> track-Hoang-address-CO.txt
        echo "Sai info" >> track-Hoang-status-CO.txt
        echo "Sai info" >> track-Hoang-number-CO.txt
        echo "Sai info" >> track-Hoang-url-CO.txt
        echo "Sai info" >> track-Hoang-categoriesType-CO.txt
    else
        sum=`md5 track-Hoang-OrderID-CO.txt`
        for (( i=0; i<=4; i++ ))
        do
            for (( j=0; j<=4; j++))
            do
                if [[ `echo $curl | jq '.data.guestOrder.groups_2101['${i[@]}'].items['${j[@]}'].productInfo.usItemId'` == '"'${array[3]}'"' ]]; then
                    echo ${array[2]} >> track-Hoang-OrderID-CO.txt
                    echo ${array[1]} >> track-Hoang-email-CO.txt
                    echo $curl | jq '.data.guestOrder.groups_2101['${i[@]}'].items['${j[@]}'].productInfo.usItemId' >> track-Hoang-usItemId-CO.txt
                    echo $curl | jq '.data.guestOrder.groups_2101['${i[@]}'].items['${j[@]}'].priceInfo.linePrice.value' >> track-Hoang-priceValue-CO.txt
                    echo $curl | jq '.data.guestOrder.groups_2101['${i[@]}'].deliveryAddress.fullName' >> track-Hoang-fullName-CO.txt
                    echo $curl | jq '.data.guestOrder.groups_2101['${i[@]}'].deliveryAddress.address.addressString' >> track-Hoang-address-CO.txt
                    echo $curl | jq '.data.guestOrder.groups_2101['${i[@]}'].status.message.parts[].text' >> track-Hoang-status-CO.txt
                    echo $curl | jq '.data.guestOrder.groups_2101['${i[@]}'].shipment.trackingNumber' | sed 's/null//g' >> track-Hoang-number-CO.txt
                    echo $curl | jq '.data.guestOrder.groups_2101['${i[@]}'].shipment.trackingUrl' | sed 's/null//g' >> track-Hoang-url-CO.txt
                    echo $curl | jq '.data.guestOrder.groups_2101['${i[@]}'].categories[].type' >> track-Hoang-categoriesType-CO.txt
                    break
                fi
            done
        done
        if [[ $sum == `md5 track-Hoang-OrderID-CO.txt` ]]; then
            echo ${array[1]}"-Khác Item"
            echo "'"${array[2]} >> track-Hoang-OrderID-CO.txt
            echo ${array[1]} >> track-Hoang-email-CO.txt
            echo "Khác Item" >> track-Hoang-usItemId-CO.txt
            echo "Khác Item" >> track-Hoang-priceValue-CO.txt
            echo "Khác Item" >> track-Hoang-fullName-CO.txt
            echo "Khác Item" >> track-Hoang-address-CO.txt
            echo "Khác Item" >> track-Hoang-status-CO.txt
            echo "Khác Item" >> track-Hoang-number-CO.txt
            echo "Khác Item" >> track-Hoang-url-CO.txt
            echo "Khác Item" >> track-Hoang-categoriesType-CO.txt
        else
            echo ${array[0]}
        fi   
...
linkPost=$linkApi`echo Item ID==IMPORTDATA\(\"`$linkGit`echo track-Hoang-usItemId-CO.txt\"\)\&Giá==IMPORTDATA\(\"`$linkGit`echo track-Hoang-priceValue-CO.txt\"\)\
\&Tên==IMPORTDATA\(\"`$linkGit`echo track-Hoang-fullName-CO.txt\"\)\&Địa chỉ==index\(IMPORTDATA\(\"`$linkGit`echo track-Hoang-address-CO.txt\"\),,1\)\
\&Status==IMPORTDATA\(\"`$linkGit`echo track-Hoang-status-CO.txt\"\)\
\&Tracking Number==IMPORTDATA\(\"`$linkGit`echo track-Hoang-number-CO.txt\"\)\
\&Tracking URL==IMPORTDATA\(\"`$linkGit`echo track-Hoang-url-CO.txt\"\)\
\&Email==IMPORTDATA\(\"`$linkGit`echo track-Hoang-email-CO.txt\"\)\
\&Order==IMPORTDATA\(\"`$linkGit`echo track-Hoang-OrderID-CO.txt\"\)\
\&categoriesType==IMPORTDATA\(\"`$linkGit`echo track-Hoang-categoriesType-CO.txt\"\)\
`
echo $linkPost