...
do
  ...
  #outline
  echo $outline | jq '.data.lesson_details[].data.tasks[] | select(.task_description == "NẠP CỤM") | .task_details[] | {'lesson_id_$lesson_id': .null, typeOutline_NAP_CUM: .null, en: .sentence_en, ipa: .ipa, vi: .sentence_vi, audio: .audio}' >> outline.txt
  echo $outline | jq '.data.lesson_details[].data.tasks[] | select(.task_description == "GHÉP CÂU") | .task_details[] | {typeOutline_GHEP_CAU: .null, en: .sentence_en, ipa: .ipa, vi: .sentence_vi, audio: .audio}' >> outline.txt

  # #basic
  echo $basic | jq '.data.lesson_details[] | select(.type == "FLEXIBLE_PHRASE") | .data | {typeBasic_FLEXIBLE_PHRASE: .null, en: .sentence_en, ipa: .ipa, vi: .sentence_vi, audio: .sentence_audio_speaker}' >> outline.txt
  echo $basic | jq '.data.lesson_details[] | select(.type == "EXTEND_IDEA_QNA") | .data.messages[] | select(.type == "text") | {typeBasic_EXTEND_IDEA_QNA: .null, en: .content, ipa: .ipa, vi: .sentence_vi, audio: .audio}' >> outline.txt
  echo $basic | jq '.data.lesson_details[] | select(.type == "LEARNING_CARD") | .data | {typeBasic_LEARNING_CARD: .null, en: .sentence_en, ipa: .ipa, vi: .sentence_vi, audio: .sentence_audio}' >> outline.txt

  #extend
  echo $extend | jq '.data.lesson_details[] | select(.type == "SPEAKING_WORD") | .data | {typeExtend_SPEAKING_WORD: .null, en: .word, ipa: .pronunciation, vi: .meaning_vi, audio: .audio}' >> outline.txt
done
