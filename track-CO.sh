#!/bin/bash
git pull
: > track-usItemId-CO.txt
: > track-priceValue-CO.txt
: > track-fullName-CO.txt
: > track-address-CO.txt
: > track-status-CO.txt
: > track-number-CO.txt
: > track-url-CO.txt
: > track-OrderID-CO.txt
: > track-email-CO.txt
: > track-categoriesType-CO.txt
...
do
  ...
  while true
    ...
    curl=`curl -s 'https://www.walmart.com/orchestra/home/graphql' \
      ...
      # echo $curl
      # echo $curl | jq '.data.guestOrder.groups_2101[].items[].productInfo.usItemId'
      # echo '"'${array[4]}'"'
      # sleep 10
    ...
    if [[ $curl =~ "error" ]]; then
        echo "1-Sai info"
        echo ${array[2]} >> track-OrderID-CO.txt
        echo ${array[1]} >> track-email-CO.txt
        echo "Sai info" >> track-usItemId-CO.txt
        echo "Sai info" >> track-priceValue-CO.txt
        echo "Sai info" >> track-fullName-CO.txt
        echo "Sai info" >> track-address-CO.txt
        echo "Sai info" >> track-status-CO.txt
        echo "Sai info" >> track-number-CO.txt
        echo "Sai info" >> track-url-CO.txt
        echo "Sai info" >> track-categoriesType-CO.txt
    else
        sum=`md5 track-OrderID-CO.txt`
        for (( i=0; i<=4; i++ ))
        do
            for (( j=0; j<=4; j++))
            do
                if [[ `echo $curl | jq '.data.guestOrder.groups_2101['${i[@]}'].items['${j[@]}'].productInfo.usItemId'` == '"'${array[3]}'"' ]]; then
                    echo ${array[2]} >> track-OrderID-CO.txt
                    echo ${array[1]} >> track-email-CO.txt
                    echo $curl | jq '.data.guestOrder.groups_2101['${i[@]}'].items['${j[@]}'].productInfo.usItemId' >> track-usItemId-CO.txt
                    echo $curl | jq '.data.guestOrder.groups_2101['${i[@]}'].items['${j[@]}'].priceInfo.linePrice.value' >> track-priceValue-CO.txt
                    echo $curl | jq '.data.guestOrder.groups_2101['${i[@]}'].deliveryAddress.fullName' >> track-fullName-CO.txt
                    echo $curl | jq '.data.guestOrder.groups_2101['${i[@]}'].deliveryAddress.address.addressString' >> track-address-CO.txt
                    echo $curl | jq '.data.guestOrder.groups_2101['${i[@]}'].status.message.parts[].text' >> track-status-CO.txt
                    echo $curl | jq '.data.guestOrder.groups_2101['${i[@]}'].shipment.trackingNumber' | sed 's/null//g' >> track-number-CO.txt
                    echo $curl | jq '.data.guestOrder.groups_2101['${i[@]}'].shipment.trackingUrl' | sed 's/null//g' >> track-url-CO.txt
                    echo $curl | jq '.data.guestOrder.groups_2101['${i[@]}'].categories[].type' >> track-categoriesType-CO.txt
                    break
                fi
            done
        done
        if [[ $sum == `md5 track-OrderID-CO.txt` ]]; then
            echo ${array[1]}"-Khác Item"
            echo "'"${array[2]} >> track-OrderID-CO.txt
            echo ${array[1]} >> track-email-CO.txt
            echo "Khác Item" >> track-usItemId-CO.txt
            echo "Khác Item" >> track-priceValue-CO.txt
            echo "Khác Item" >> track-fullName-CO.txt
            echo "Khác Item" >> track-address-CO.txt
            echo "Khác Item" >> track-status-CO.txt
            echo "Khác Item" >> track-number-CO.txt
            echo "Khác Item" >> track-url-CO.txt
            echo "Khác Item" >> track-categoriesType-CO.txt
        else
            echo ${array[0]}
        fi   
