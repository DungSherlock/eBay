...
curl2=`curl 'https://www.walmart.com/orchestra/home/graphql' \
  ...
  echo $curl2 | jq '.data.guestOrder.groups_2101[].items[].product.usItemId' >> track-usItemId.txt
  echo $curl2 | jq '.data.guestOrder.groups_2101[].items[].priceInfo.linePrice.value' >> track-priceValue.txt
