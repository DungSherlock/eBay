...
if [[ $curl35 =~ "error" ]]; then
    ...
    do
        for (( j=0; j<=4; j++))
        do
            if [[ `echo $curl35 | jq '.data.guestOrder.groups_2101['${i[@]}'].items['${j[@]}'].productInfo.usItemId'` == '"223404599"' ]]; then
                echo "'3382220255830" >> track-OrderID.txt
                echo Sarah7hone@gmail.com >> track-email.txt
...
if [[ $curl37 =~ "error" ]]; then
    ...
    do
        for (( j=0; j<=4; j++))
        do
            if [[ `echo $curl37 | jq '.data.guestOrder.groups_2101['${i[@]}'].items['${j[@]}'].productInfo.usItemId'` == '"599495878"' ]]; then
                echo "'3402223129589" >> track-OrderID.txt
                echo Mikefrederickltd@msn.com >> track-email.txt
...
if [[ $curl39 =~ "error" ]]; then
    ...
    do
        for (( j=0; j<=4; j++))
        do
            if [[ `echo $curl39 | jq '.data.guestOrder.groups_2101['${i[@]}'].items['${j[@]}'].productInfo.usItemId'` == '"16481007"' ]]; then
                echo "'3402223843524" >> track-OrderID.txt
                echo destinyangeliqueeee@icloud.com >> track-email.txt
