...
if [[ $curl5 =~ "error" ]]; then
    ...
    do
        for (( j=0; j<=4; j++))
        do
            if [[ `echo $curl5 | jq '.data.guestOrder.groups_2101['${i[@]}'].items['${j[@]}'].productInfo.usItemId'` == '"188433577"' ]]; then
                echo "'3942254078152" >> track-OrderID.txt
                echo southernjean@hotmail.com >> track-email.txt
...
if [[ $curl9 =~ "error" ]]; then
    ...
    do
        for (( j=0; j<=4; j++))
        do
            if [[ `echo $curl9 | jq '.data.guestOrder.groups_2101['${i[@]}'].items['${j[@]}'].productInfo.usItemId'` == '"377870589"' ]]; then
                echo "'3942254779131" >> track-OrderID.txt
                echo judyt317@gmail.com >> track-email.txt
...
if [[ $curl13 =~ "error" ]]; then
    ...
    do
        for (( j=0; j<=4; j++))
        do
            if [[ `echo $curl13 | jq '.data.guestOrder.groups_2101['${i[@]}'].items['${j[@]}'].productInfo.usItemId'` == '"285333157"' ]]; then
                echo "'3942254784218" >> track-OrderID.txt
                echo cindibutcher@gmail.com >> track-email.txt
