...
if [[ $curl26 =~ "error" ]]; then
    ...
    do
        for (( j=0; j<=4; j++))
        do
            if [[ `echo $curl26 | jq '.data.guestOrder.groups_2101['${i[@]}'].items['${j[@]}'].productInfo.usItemId'` == '"55036486"' ]]; then
                echo "'3522244373098" >> track-OrderID-ADOT.txt
                echo mwflen@bellsouth.net >> track-email-ADOT.txt
...
if [[ $curl30 =~ "error" ]]; then
    ...
    do
        for (( j=0; j<=4; j++))
        do
            if [[ `echo $curl30 | jq '.data.guestOrder.groups_2101['${i[@]}'].items['${j[@]}'].productInfo.usItemId'` == '"292977630"' ]]; then
                echo "'3522244371752" >> track-OrderID-ADOT.txt
                echo rickimjes@yahoo.com >> track-email-ADOT.txt
