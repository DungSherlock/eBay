...
for (( i=0; i<=4; i++ ))
  ...
  if [[ $curl18 =~ "error" ]]; then
    ...
    for (( j=0; j<=4; j++))
    do
      if [[ `echo $curl18 | jq '.data.guestOrder.groups_2101['${i[@]}'].items['${j[@]}'].productInfo.usItemId'` == '"105489036"' ]]; then
...
for (( i=0; i<=4; i++ ))
  ...
  if [[ $curl19 =~ "error" ]]; then
    ...
    for (( j=0; j<=4; j++))
    do
      if [[ `echo $curl19 | jq '.data.guestOrder.groups_2101['${i[@]}'].items['${j[@]}'].productInfo.usItemId'` == '"45465229"' ]]; then
