...
curl2=`curl 'https://www.walmart.com/orchestra/home/graphql' \
  ...
  echo $curl2 | jq '.data.guestOrder.groups_2101[].items[2].product.usItemId' >> track-usItemId.txt
  echo $curl2 | jq '.data.guestOrder.groups_2101[].items[2].priceInfo.linePrice.value' >> track-priceValue.txt
  echo $curl2 | jq '.data.guestOrder.groups_2101[].deliveryAddress.fullName' >> track-fullName.txt
  echo $curl2 | jq '.data.guestOrder.groups_2101[].deliveryAddress.address.addressString' >> track-address.txt
  echo $curl2 | jq '.data.guestOrder.groups_2101[].status.message.parts[].text' >> track-status.txt
  echo $curl2 | jq '.data.guestOrder.groups_2101[].shipment.trackingNumber' >> track-number.txt
  echo $curl2 | jq '.data.guestOrder.groups_2101[].shipment.trackingUrl' >> track-url.txt
