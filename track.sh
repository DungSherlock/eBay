...
if [[ $curl54 =~ "error" ]]; then
    ...
    do
        for (( j=0; j<=4; j++))
        do
            if [[ `echo $curl54 | jq '.data.guestOrder.groups_2101['${i[@]}'].items['${j[@]}'].productInfo.usItemId'` == '"300578963"' ]]; then
                echo "'3952256583644" >> track-OrderID.txt
                echo ADOT220425204845111 >> track-email.txt
...
if [[ $curl55 =~ "error" ]]; then
    ...
    echo "ADOT220427001908682" >> track-email.txt
    echo "Sai info" >> track-usItemId.txt
    echo "Sai info" >> track-priceValue.txt
    echo "Sai info" >> track-fullName.txt
    echo "Sai info" >> track-address.txt
    ...
    do
        for (( j=0; j<=4; j++))
        do
            if [[ `echo $curl55 | jq '.data.guestOrder.groups_2101['${i[@]}'].items['${j[@]}'].productInfo.usItemId'` == '"377870589"' ]]; then
                echo "'3962258709795" >> track-OrderID.txt
                echo ADOT220427001908682 >> track-email.txt
