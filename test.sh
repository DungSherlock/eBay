...
do
  ...
  while true
    ...
    if [[ $curl =~ "error" ]]; then
        echo "1-Sai info"
        echo ${array[3]} >> track-OrderID-ADOT.txt
        ...
        do
            for (( j=0; j<=4; j++))
            do
                if [[ `echo $curl | jq '.data.guestOrder.groups_2101['${i[@]}'].items['${j[@]}'].productInfo.usItemId'` == ""${array[4]}"" ]]; then
                    echo ${array[3]} >> track-OrderID-ADOT.txt
