...
if [[ $curl1 =~ "error" ]]; then
    echo "1-Sai info"
    echo "'2972267729170"
    echo "kenya_adams1471@yahoo.com"
    echo "Sai info"
    echo "Sai info"
    echo "Sai info"
    echo "Sai info"
    ...
    do
        for (( j=0; j<=4; j++))
        do
            if [[ `echo $curl1 | jq '.data.guestOrder.groups_2101['${i[@]}'].items['${j[@]}'].productInfo.usItemId'` == '"10752553"' ]]; then
                echo "'2972267729170"
                echo kenya_adams1471@yahoo.com
                ...
                echo $curl1 | jq '.data.guestOrder.groups_2101['${i[@]}'].categories[].type'
                break
            fi
        done
    done
