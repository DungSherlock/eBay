...
do
  ...
  while true
    ...
    if [[ $curl =~ "error" ]]; then
        echo "1-Sai info"
        echo ${arrLINE[3]} >> track-OrderID-ADOT.txt
        echo ${arrLINE[1]} >> track-email-ADOT.txt
        ...
        do
            for (( j=0; j<=4; j++))
            do
                if [[ `echo $curl | jq '.data.guestOrder.groups_2101['${i[@]}'].items['${j[@]}'].productInfo.usItemId'` == "'"${arrLINE[4]}"'" ]]; then
                    echo ${arrLINE[3]} >> track-OrderID-ADOT.txt
